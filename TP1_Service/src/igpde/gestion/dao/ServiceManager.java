...
public class ServiceManager {
  ...
  public Service serviceUpdate (Service service)
  {
	  ...
	  try {
		  tx = s.beginTransaction();
		  System.out.println();
		  Query query = s.createQuery("update Service set interlocuteur = 'DIALOG2',libelle='COUCOU'" +
...
public Service serviceGet (int id)
{
	  Service service = null;
		Session s = sessionFactory.openSession();
		Transaction tx = null;
...
public Service serviceLoad (int id)
{
	  ...
	  try {
		  tx = s.beginTransaction();
		  service=(Service) s.load(Service.class, id);
...
public Service serviceMAJ (int id)
{
	  Service service = null;
		Session s = sessionFactory.openSession();
		Transaction tx = null;
	  try {
		  tx = s.beginTransaction();
		  service=(Service) s.get(Service.class, id);
		  service.setInterlocuteur("TOTO");
		  service.setLibelle("TATA");
		  s.update(service);
	} catch (Exception e)  {
		// TODO: handle exception
		if (tx != null)
